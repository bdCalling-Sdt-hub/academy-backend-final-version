...
class WebsiteController extends Controller
{
    ...
    {
        $query = Batch::with('course.course_category', 'course.course_module.videos', 'teachers','course.reviews.user');
        ...
        if ($request->filled('course_id')) {
            $query->whereHas('course.modules', function ($q) use ($request) {
                $q->where('id', $request->course_id);
            });
        }

        if ($request->filled('popular_section')) {
            $query->whereHas('course', function ($q) use ($request) {
                $q->where('popular_section', $request->popular_section);
            });
        }
        ...
        $filter_course = $query->paginate($request->per_page ?? 10);
