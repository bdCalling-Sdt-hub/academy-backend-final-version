...
use App\Models\User;
use App\Services\CourseEnrollAuthorizationService;
use Carbon\Carbon;
...
class AddEmployeeController extends Controller
{
    ...
    public function __construct(CourseEnrollAuthorizationService $enrollService)
    {
    ...
    public function courseEnrollAuthorization(Request $request)
    {
        try {
            $course_enroll =  $this->enrollService->courseEnrollService($request);
            return response()->json(['message' => 'Course Enroll Services', 'data' => $course_enroll]);
        }catch(\Exception $e){
            return response()->json(['message' => 'Course Enroll Service Fail' ,'error' => $e->getMessage()],500);
        }
    }
