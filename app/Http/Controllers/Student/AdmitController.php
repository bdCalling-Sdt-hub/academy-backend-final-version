    ...
    public function showAdmitStudentV2(Request $request)
    {
        ...
        $query = BatchStudent::with(['batch.course', 'student.user'])
            ...
            ->map(function ($student) {
                ...
                $payment_info = Order::where('student_id', $student->student_id)
        ...
        return response()->json(['enrolled_student_list' => $query]);
    }

    public function showAdmitStudent(Request $request)
    {
        // Get the list of enrolled students
        $enrolled_students = BatchStudent::where('status', 'enrolled')->get();

        // Extract the student IDs of the enrolled students
        $dropout_student_ids = $enrolled_students->pluck('student_id')->toArray();
